...
from infinigen.lidar.lidar_engine import LidarConfig, compute_intensity
...
def test_coverage_scales_intensity():
    ...
    base = _props()
    base["opacity"] = 1.0
    I_full, _, _, a_full = compute_intensity(base, cos_i=1.0, R=3.0, cfg=cfg)
    base["opacity"] = 0.25
    I_quarter, _, _, a_quarter = compute_intensity(base, cos_i=1.0, R=3.0, cfg=cfg)
...
def test_transmission_reduces_reflectivity_and_residual_positive():
    cfg = LidarConfig()
    # Returns: (intensity, reflectivity, T_mat, alpha_cov)
    _, refl0, T0, _ = compute_intensity(
        _props(transmission=0.0), cos_i=0.95, R=3.0, cfg=cfg
    )
    _, refl1, T1, _ = compute_intensity(
        _props(transmission=0.85), cos_i=0.95, R=3.0, cfg=cfg
    )
    # refl1 (transmissive) < refl0 (opaque)
    # T1 (transmissive) > T0 (opaque)
    assert refl1 < refl0 and T1 > T0
...
def test_metallic_mixing_increases_fresnel():
    ...
    _, refl_d, *_ = compute_intensity(dielec, cos_i=0.95, R=3.0, cfg=cfg)
    _, refl_m, *_ = compute_intensity(metal, cos_i=0.95, R=3.0, cfg=cfg)
...
def test_ior_produces_higher_f0_than_low_specular():
    ...
    _, refl1, *_ = compute_intensity(p1, cos_i=0.98, R=3.0, cfg=cfg)
    _, refl2, *_ = compute_intensity(props, cos_i=0.98, R=3.0, cfg=cfg)
    assert refl1 > refl2


def test_energy_conservation_bound():
    cfg = LidarConfig()
    _, refl, Tmat, _ = compute_intensity(
...
def test_alpha_coverage_scales_reported():
    ...
    props = _props()
    props["opacity"] = 0.4
    I0, _, _, alpha_cov = compute_intensity(props, cos_i=0.9, R=4.0, cfg=cfg)
