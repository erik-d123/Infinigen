...
def _read_ply_ascii_tolerant(path: Path) -> dict:
    ...
    with open(path, "r", encoding="utf-8") as f:
        ...
        for line in f:
            ...
            for j, (name, dtype) in enumerate(props):
                tok = toks[j]
                if dtype in {"uchar","char","ushort","short","uint","int","uint8","uint16"}:
                    try:
                        v = int(tok)
                    except ValueError:
                        v = int(float(tok))
    ...
    def arr(name): return np.asarray(cols[name]) if name in cols else None
    ...
    for name, dt in [
        ...
        ("range_m","f4"), ("cos_incidence","f4"),
        ("mat_class","u1"), ("reflectivity","f4"), ("transmittance","f4"),
    ]:
        put(name, dt)
    if all(k in cols for k in ("nx","ny","nz")):
        out["normals"] = np.stack([arr("nx"),arr("ny"),arr("nz")], axis=1).astype("f4")
    return out
...
def _read_ply_all(path: Path) -> Dict[str, np.ndarray]:
    ...
    if PlyData is not None:
        ...
        def col(name, dtype=None):
            ...
            return None
        x = col("x","f4"); y = col("y","f4"); z = col("z","f4")
        ...
        if all(k in data for k in ("nx","ny","nz")):
            data["normals"] = np.stack([data.pop("nx"), data.pop("ny"), data.pop("nz")], axis=1).astype("f4")
        return data
...
def _intensity_to_heat(intensity: np.ndarray, N: int) -> np.ndarray:
    ...
    def lerp(x, a, b):
        return a + (b - a) * x
    R = np.where(g < 0.5, 0.0, np.where(g < 0.75, lerp((g-0.5)/0.25, 0.0, 1.0), 1.0))
    G = np.where(g < 0.25, lerp(g/0.25, 0.0, 1.0), np.where(g < 0.75, 1.0, lerp((g-0.75)/0.25, 1.0, 0.0)))
    B = np.where(g < 0.25, 1.0, np.where(g < 0.5, lerp((g-0.25)/0.25, 1.0, 0.0), 0.0))
...
def _load_frame(path: Path, mode: str) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray], Dict[str,np.ndarray]]:
...
def view_dir(out_dir: str, color: str = "intensity", frame: Optional[int]=None):
    ...
    mode = color if color in ("intensity","intensity_heat","reflectivity","ring") else "intensity"
    pts, cols, norms, aux = _load_frame(files[idx], mode)
    pcd = _make_o3d_pcd(pts, cols, norms)

    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window(window_name=_title(outp, files, idx, mode), width=1280, height=800)
    ...
    vis.register_key_callback(ord('C'), lambda v: reload_geometry(idx, {m for m in []} or ['intensity','intensity_heat','reflectivity','ring'][(['intensity','intensity_heat','reflectivity','ring'].index(mode)+1)%4]))
...
def parse_args(argv=None):
    ...
    ap.add_argument("--color", type=str, default="intensity", choices=["intensity","intensity_heat","reflectivity","ring"])
